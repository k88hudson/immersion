...
/***/ function(module, exports, __webpack_require__) {
	...
	var Medium = __webpack_require__(1);
	
	var lines = [['今年カナダに母の日は ５月１０日です。', 'カナダでは、今年の母の日は５月１０日です。'], ['母の日に日本人と同様にカナダ人が母にプレゼントをあげます。', '母の日には日本人と同様に、カナダ人は母にプレゼントをあげます。'], ['其れにカードを書きます、そして、レストランで食べたり料理したりする。', 'そして、カードも書きます/添えます。レストランで食べたり、家で料理を作ったりすることもあります。'], ['去年私の家族はイタリアのレストランへ行きました。', '去年、私の家族はイタリアのレストランへ行きました。'], ['パスタを食べました、ワインレッドを飲みました。', 'パスタを食べました、ワインレッドを飲みました。'], ['残念ことに今年はお父が亡くなりましたので、母が悲しいです。', '残念なことに、今年はお父が亡くなりましたので、母はとても悲しそうです。'], ['したが, 母の日私は綺麗なプレゼントをあげたいです、一番美味料理をしたいです。', 'だから、 母の日に私はすてきなプレゼントをあげたいです。そして一番美味しい料理を作りたいです。'], ['幸せになりたいよ！', '母を幸せにしたいです！']];
	...
	var Editor = React.createClass({ displayName: 'Editor',
	  ...
	  componentDidMount: function componentDidMount() {
	    this.medium = new Medium({
	      ...
	      keyContext: {
	        enter: function enter(e, element) {
	          element.blur();
	        }
	      }
	    });
	  ...
	  makeLi: function makeLi(part) {
	    var className = '';
	    if (part.added) className = 'added';else if (part.removed) className = 'removed';
	    return React.createElement('li', { className: className }, part.value);
	  },
	  getDiff: function getDiff() {
	    var parts = diff.diffWords(this.props.originalText, this.state.currentText);
	    return React.createElement('ul', { className: 'diff' }, parts.map(this.makeLi));
	  },
	  ...
	  showEditor: function showEditor(show) {
	    var _this = this;
	  ...
	  render: function render() {
	    return React.createElement('div', { className: 'line-container' }, React.createElement('div', { className: 'line-editor', ref: 'editor', onClick: this.onClick, onKeyUp: this.onChange, onBlur: this.showEditor(false) }, this.props.originalText), React.createElement('div', { className: 'line' }, this.getDiff()));
	  }
	});
	
	var Eg = React.createClass({ displayName: 'Eg',
	  getInitialState: function getInitialState() {
	    return {
	      showRemoved: true
	    };
	  ...
	  makeLine: function makeLine(line) {
	    var parts = diff.diffChars(line[0], line[1]);
	    return React.createElement('ul', { className: 'diff' }, parts.map(this.makeLi));
	  },
	  toggleRemoved: function toggleRemoved(e) {
	    e.preventDefault();
	  ...
	  render: function render() {
	    ...
	    return React.createElement('div', { className: className }, React.createElement('img', { src: './img/mother.jpg' }), React.createElement('h3', null, 'Mother’s Day'), en.map(function (line, i) {
	      return React.createElement(Editor, { key: i, originalText: line });
	    }));
	  }
	});
	
	React.render(React.createElement('div', null, React.createElement(Eg, null)), document.getElementById('app'));
	...
	(function (w, d) {
		...
		var Medium = (function () {
			...
			var trim = function trim(string) {
			    ...
			    arrayContains = function arrayContains(array, variable) {
				var i = array.length;
				while (i--) {
					if (array[i] === variable) {
						return true;
	   ...
	   */
			Medium = (function (_Medium) {
				function Medium(_x) {
					return _Medium.apply(this, arguments);
				}
	
				Medium.toString = function () {
					return _Medium.toString();
				};
				...
				var medium = this,
					...
					up: function up(e) {
						...
						var keyContext;
						if (settings.keyContext !== null && (keyContext = settings.keyContext[e.keyCode])) {
					...
					command: {
						bold: function bold(e) {
							...
							new Medium.Element(medium, 'bold').setClean(false).invoke(settings.beforeInvokeElement);
						},
						underline: function underline(e) {
							utils.preventDefaultEvent(e);
							new Medium.Element(medium, 'underline').setClean(false).invoke(settings.beforeInvokeElement);
						},
						italicize: function italicize(e) {
							utils.preventDefaultEvent(e);
							new Medium.Element(medium, 'italic').setClean(false).invoke(settings.beforeInvokeElement);
						},
						...
						paste: function paste(e) {
							...
							if (settings.pasteAsText) {
								...
								utils.pasteHook(function (text) {
									...
									new Medium.Html(medium, text).setClean(false).insert(settings.beforeInsertHtml, true);
					...
					enterKey: function enterKey(e) {
						...
						if (!cache.shift) {
							...
							if (lastChild && lastChild !== el.firstChild && settings.autoHR && settings.mode !== 'partial' && settings.tags.horizontalRule) {
	
								utils.preventDefaultEvent(e);
	
								makeHR = html.text(lastChild) === '' && lastChild.nodeName.toLowerCase() === settings.tags.paragraph;
					...
					backspaceOrDeleteKey: function backspaceOrDeleteKey(e) {
						...
						} else if (lastChild && beforeLastChild && utils.html.text(lastChild).length < 1 && beforeLastChild.nodeName.toLowerCase() === settings.tags.horizontalRule && lastChild.nodeName.toLowerCase() === settings.tags.paragraph) {
							el.removeChild(lastChild);
					...
					pasteAsText: true,
					beforeInvokeElement: function beforeInvokeElement() {},
					beforeInsertHtml: function beforeInsertHtml() {},
					beforeAddTag: function beforeAddTag(tag, shouldFocus, isEditable, afterElement) {},
					keyContext: null,
					pasteEventHandler: function pasteEventHandler(e) {
						e = e || w.event;
						medium.makeUndoable();
				...
				for (i in defaultSettings) {
					...
					if (typeof defaultSettings[i] !== 'object' && defaultSettings.hasOwnProperty(i) && settings.element.getAttribute('data-medium-' + key)) {
	    ...
	    * @returns {Medium}
	    */
				invokeElement: function invokeElement(tagName, attributes) {
					...
					new Medium.Element(this, tagName, attributes).invoke(this.settings.beforeInvokeElement);
	    ...
	    */
				...
				destroy: function destroy() {
					...
					//remove classes
					el.className = trim(el.className.replace(settings.cssClasses.editor, '').replace(settings.cssClasses.clear, '').replace(settings.cssClasses.editor + '-' + settings.mode, ''));
	
					//remove events
					this.utils.removeEvent(el, 'keyup', intercept.up).removeEvent(el, 'keydown', intercept.down).removeEvent(el, 'focus', intercept.focus).removeEvent(el, 'blur', intercept.focus).removeEvent(el, 'paste', settings.pasteEventHandler);
				},
	   ...
	   */
	     ...
	     * @returns {null}
	     */
					inject: function inject(htmlRaw, selectInserted) {
	    ...
	    */
	     ...
	     */
					setClean: function setClean(clean) {
						this.clean = clean;
						return this;
					}
				};
	     ...
	     * @returns {HtmlElement}
	     */
					inject: function inject(htmlRaw) {
						var html,
						    isConverted = false;
	    ...
	    */
				Medium.Undoable = function (medium) {
					var me = this,
						...
						constructor: function constructor(oldValue, newValue) {
					    ...
					    makeUndoable = function makeUndoable() {
			...
			Medium.Html.prototype = {
				setBridge: function setBridge(bridge) {
					for (var i in bridge) {
						this[i] = bridge[i];
					}
				},
	    ...
	    * @returns {HtmlElement}
	    */
				insert: function insert(fn, selectInserted) {
					if (Medium.activeElement === this.element) {
						if (fn) {
							fn.apply(this);
						}
	    ...
	    */
				isCommand: function isCommand(e, fnTrue, fnFalse) {
					var s = this.settings;
					if (s.modifier === 'ctrl' && e.ctrlKey || s.modifier === 'cmd' && e.metaKey || s.modifier === 'auto' && (e.ctrlKey || e.metaKey)) {
						return fnTrue.call();
					} else {
						return fnFalse.call();
					}
				},
				isShift: function isShift(e, fnTrue, fnFalse) {
					if (e.shiftKey) {
						return fnTrue.call();
					} else {
						return fnFalse.call();
				...
				isModifier: function isModifier(e, fn) {
	    ...
	    */
				pasteHook: function pasteHook(fn) {
				...
				setupContents: function setupContents() {
					var el = this.element,
					    children = el.children,
					    childNodes = el.childNodes,
					    initialParagraph;
	
					if (!this.settings.tags.paragraph || children.length > 0 || this.settings.mode === Medium.inlineMode) {
						return;
					}
				...
				traverseAll: function traverseAll(element, options, depth) {
	   ...
	   */
			...
			Medium.Cursor.prototype = {
				...
				caretToBeginning: function caretToBeginning(el) {
					this.set(0, el);
				},
				caretToEnd: function caretToEnd(el) {
	   ...
	   */
			...
			Medium.HtmlAssistant.prototype = {
				...
				encodeHtml: function encodeHtml(html) {
					return d.createElement('a').appendChild(d.createTextNode(html)).parentNode.innerHTML;
				},
				...
				changeTag: function changeTag(oldNode, newTag) {
				...
				deleteNode: function deleteNode(el) {
	    ...
	    */
	     ...
	     */
					...
					this.utils.traverseAll(el, {
						element: function element(child, i, depth, parent) {
				...
				addTag: function addTag(tag, shouldFocus, isEditable, afterElement) {
				...
				baseAtCaret: function baseAtCaret() {
			...
			Medium.Action.prototype = {
				...
				listen: function listen() {
					...
					this.utils.addEvent(el, 'keyup', intercept.up).addEvent(el, 'keydown', intercept.down).addEvent(el, 'focus', intercept.focus).addEvent(el, 'blur', intercept.blur).addEvent(el, 'paste', this.settings.pasteEventHandler);
				},
				preserveElementFocus: function preserveElementFocus() {
	...
	(function (global, undefined) {
	  ...
	  var JsDiff = (function () {
	    ...
	    return {
	      ...
	      diffChars: function diffChars(oldStr, newStr, callback) {
	        return CharDiff.diff(oldStr, newStr, callback);
	      },
	      diffWords: function diffWords(oldStr, newStr, callback) {
	        return WordDiff.diff(oldStr, newStr, callback);
	      },
	      diffWordsWithSpace: function diffWordsWithSpace(oldStr, newStr, callback) {
	        return WordWithSpaceDiff.diff(oldStr, newStr, callback);
	      },
	      diffLines: function diffLines(oldStr, newStr, callback) {
	        return LineDiff.diff(oldStr, newStr, callback);
	      },
	      diffTrimmedLines: function diffTrimmedLines(oldStr, newStr, callback) {
	      ...
	      diffCss: function diffCss(oldStr, newStr, callback) {
	        return CssDiff.diff(oldStr, newStr, callback);
	      },
	      diffJson: function diffJson(oldObj, newObj, callback) {
